...
import base64
from yolov5.utils.general import xyxy2xywh,increment_path,non_max_suppression
...
#cv2.VideoCapture(cv2.CAP_ANY).set(cv2.CAP_PROP_BACKEND, cv2.CAP_FFMPEG)
#import redis
lock = threading.Lock()
...
@api_view(['GET','POST'])
def index(request): 
    global ipcamera_url,model
    global dettype,apihit,model_1,model_2
    #initialize_models()
    if apihit is not None:
        print('api is already working')
        apihit = None
        return render(request,'index.html',{'error':'API IS ALREADY WORKING,TRY AGAIN'})
...
#global json_value
json_value = {}
json_value["allplates"] = []
# #out = cv2.VideoWriter('drone_detection.avi',cv2.VideoWriter_fourcc(*'XVID'), 20, (1366,720))
seen = 0
@torch.no_grad()
def stream(request):
    global frame,ipcamera_url,frame2
    global seen,model_1,model_2,json_value 
    global data,mydata,mydata2
    global btnval,apihit,cap
    mydata = {}
    btnval = 'false'
    streaming = True
    print(ipcamera_url)
    ...
    while streaming: 
        ...
        #print(ipcamera_url) 
        #time.sleep(0.01)
        if ret and frame.shape is not None:
            frame = cv2.resize(frame,(1280,720),interpolation=cv2.INTER_AREA)
            frame2 = frame.copy()
            if frame.shape[2] != 3:
                frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            #frame = resize(frame,(640,640))
            #frame = to_tensor(frame).unsqueeze(0).float().div(255.0).to(device)
            #frame1 = frame.copy()
            #frame = torch.from_numpy(frame).permute(2, 0, 1).unsqueeze(0).float().div(255.0).to(device)
            ...
            if dettype is not None and dettype=='lic':
                ...
                names = model_1.names
                results = model_1(frame,augment = True)
                #print(results)
                #for i,det in enumerate(results):
                #det = results[0].squeeze(0)

                #print(det)
                #det = results[0][0].detach().cpu().numpy()
                det = results.pred[0]
                #print(det)
                ...
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det): 
                        ...
                        imc = save_one_box(xyxy, imc, file=save_dir / f'{seen}.jpg', BGR=True,save=False)
                        # histogram equalization  
                        #print(imc.shape)
                        imc=imc[:,:,::-1]
                        #equ = cv2.equalizeHist(imc)
# Gaussian blur
...
# manual thresholding
                        ...
                        #equ[equ<th2]  = 0
                        try:
                            Result = reader.readtext(imc, detail=0)
                            print(Result)
                ...
                # print(frame.shape)
                json_value["allplates"] = all_results                
...
def stop_video(request):
    global btnval,apihit,streaming
   # if request.method=='POST':
    with lock:
        apihit = None
    ...
    #btnval = btndata['stop']
       # if btnval=='true':
        streaming = False
        #cap.release()
        print(btnval, 'streaming stopped')
